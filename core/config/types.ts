...
declare global {
  ...
  export interface ContinueUIConfig {
    ...
    /**
     * If true, keep the user's selected model per role (Chat, Autocomplete,
     * Edit, Apply, Embed, Rerank) even when that model is temporarily
     * unavailable. Instead of auto-switching to another model, the selection
     * is preserved and an error is surfaced when the model is actually used.
     * The original selection auto-recovers once the model is available again.
     */
    disableModelAutoSwitch?: boolean;
    /**
     * 历史会话懒加载：加载会话时只读取最近若干轮消息，
     * 上滑到顶部时再分页加载更早的消息。关闭时一次性加载全部。
     */
